...
export type ExternalImportFailure = { entryId:string; lemma:string|null; reason:string };
export type ExternalImportSummary = { batchId:string; destination:string; blockId:string; requested:number; validated:number; imported:number; updated:number; alreadyCurrent:number; audioGenerated:number; audioReused:number; needsPronunciationReview:string[]; warnings:string[]; failed:ExternalImportFailure[] };
export type ExternalImportEvent = { stage:string; current?:number; total?:number; entryId?:string; batchId?:string; destination?:string; reason?:string; errors?:string[]; text?:string; route?:string; path?:string };
