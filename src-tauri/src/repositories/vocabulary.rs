...
pub async fn list(
    ...
    Ok(sqlx::query_as::<_,ManagedEntry>(r#"SELECT v.id,v.word,v.ipa,v.part_of_speech,v.vi_meaning,v.en_definition,v.example_meaning_en,v.example_meaning_vi,v.example_usage_en,v.example_usage_vi,v.collocations,v.usage_note,v.register,v.word_family,v.synonyms,v.antonyms,v.accepted_answers,v.extra_metadata,v.source_key,v.source_name,v.source_level,v.source_index,v.cefr,v.audio_path,v.audio_voice,v.audio_checksum,be.id block_entry_id,be.mastery_score,be.total_reviews FROM block_entries be JOIN vocabulary_entries v ON v.id=be.entry_id WHERE be.block_id=? AND v.word LIKE ? ORDER BY v.word COLLATE NOCASE,v.id LIMIT ? OFFSET ?"#).bind(block_id).bind(q).bind(limit).bind(offset).fetch_all(pool).await?)
}
